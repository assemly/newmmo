Shader "EGA/Particles/Blend_TwoSides"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutput o )
		{
			...
			float4 staticSwitch101 = lerp(_FrontFacesColor, ( ( _FrontFacesColor * ( 1.0 - fresnelNode95 ) ) + ( _FresnelEmission * _FresnelColor * fresnelNode95 ) ), _UseFresnel);
			...
			float staticSwitch103 = lerp( 1.0, uv_Noise.z, _UseCustomData);
			clip( ( tex2D( _Mask, uv_Mask ) * tex2D( _Noise, ( (uv_Noise).xy + ( _Time.y * appendResult22 ) + uv_Noise.w ) ) * staticSwitch103 ).r - _Cutoff );
		}
