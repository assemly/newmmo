...
namespace GameServer.Services
{
    class FriendService:Singleton<FriendService>
    {
        ...
        private void OnFriendAddRequest(NetConnection<NetSession> sender, FriendAddRequest request)
        {
            ...
            {
                //如果没有传入ID,则使用名字查找
                foreach(var cha in CharacterManager.Instance.Charaters)
                {
                ...
                if (request.ToId > 0)
                {
                    if (character.FriendManager.GetFriendInfo(request.ToId) != null)
                    {
                        ...
                        sender.Session.Response.friendAddRes.Erromsg = "已经是好友了";
                        sender.SendResponse();
                        return;
                    }
                ...
                if (friend == null)
                {
                    sender.Session.Response.friendAddRes = new FriendAddResponse();
                    sender.Session.Response.friendAddRes.Result = Result.Failed;
                    sender.Session.Response.friendAddRes.Erromsg = "好友不存在或者不在线";
                    sender.SendResponse();
                    return;
                }
            }
        ...
        private void OnFriendAddResponse(NetConnection<NetSession> sender, FriendAddResponse response)
        {
            ...
            if (response.Result == Result.Success)
            {
                ...
                {
                    sender.Session.Response.friendAddRes.Result = Result.Failed;
                    sender.Session.Response.friendAddRes.Erromsg = "请求者已下线";
                }
                else
                {
            ...
            else
            {
                ...
                requester.Session.Response.friendAddRes.Erromsg = "对方拒绝添加好友";
                requester.SendResponse();
            }
