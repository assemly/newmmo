...
namespace GameServer.Managers
{
    class QuestManager
    {
        ...
        public Result AcceptQuest(NetConnection<NetSession> sender, int questId)
        {
            ...
            {
                sender.Session.Response.questAccept.Erromsg = "任务不存在";
        ...
        public Result SubmitQuest(NetConnection<NetSession> sender, int questId)
        {
            ...
            if(DataManager.Instance.Quests.TryGetValue(questId,out quest))
            {
                ...
                if(dbquest != null)
                {
                    ...
                    {
                        //还不是完成状态
                        sender.Session.Response.questSubmit.Erromsg = "任务未完成";
                        return Result.Failed;
                    }
                ...
                sender.Session.Response.questSubmit.Erromsg = "任务不存在[2]";
                return Result.Failed;
            }
            else
            {
                sender.Session.Response.questSubmit.Erromsg = "任务不存在[1]";
                return Result.Failed;
            }
        }
    }
